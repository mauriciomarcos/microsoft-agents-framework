...
public sealed class NewsletterGeneratorAgent(ILogger<NewsletterGeneratorAgent> logger,
    [FromKeyedServices(PromptProvider.File)] IPromptProvider promptProvider) : IAgent<IEnumerable<Article>, string>
{
    private const string AGENT_NAME = "NewsletterGeneratorAgent";
    ...
    public async Task<string> RunAsync(IEnumerable<Article> data, CancellationToken cancellationToken)
    {
        ...
        var agent = client
            ...
            {
                Name = AGENT_NAME,
                Description = "Agente especialista em gerar newsletter via E-mail.",
                ChatOptions = new()
                {
